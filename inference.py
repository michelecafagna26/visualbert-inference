...
for i in range(1):
    ...
    try:
        ...
        for b, (time_per_batch, (batch, tokens)) in enumerate(time_batch(val_loader if args.no_tqdm else tqdm(val_loader), reset_every=ARGS_RESET_EVERY)):
            with torch.no_grad():
                batch = _to_gpu(batch)
                output_dict = train_model.step(batch, eval_mode = True)
                print(output_dict.keys())

                index = val_loader.dataset.items[b]['image_id']
                ...
                np.savez("XX/X_features/attw/" + outfile_name, tokens = output_dict['tokens'], attention_weights = output_dict['attention_weights'] )
